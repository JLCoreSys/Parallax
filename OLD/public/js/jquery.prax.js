$(function(){
    ...
    function measureAnimations(){
        ...
        for( i=0, l=$items.length; i<l; i++ ){
            $items.eq(i).data().iTop    = Infinity;
            $items.eq(i).data().iBottom = -Infinity;
        }
        ...
        iPageHeight = Math.max( iPageHeight, ++$items.last().data().iBottom );
    ...
    function onScroll( iScrTop ){
        var bChangedLoc = false
            , i, l, oAnim, $sec, oData
            , $node, sItemId, n, oCssProps, oProps, iCurScr, sState
            ;
        ...
        for( i=0, l=$items.length; i<l; i++ ){
            ...
            if( ( oData.iTop <= iScrTop ) && ( oData.iBottom >= ( iScrTop ) ) ){
                ...
                if( !bChangedLoc ){
                    if( sLastHash != ( sItemId = $sec.attr( 'id' ).split( '-' ).pop() ) ){
                        location.replace( '#' + ( sLastHash = sItemId ) );
                        $body.prop( 'class', $body.prop( 'class' ).replace( /(?: |^)section-[^ ]+/g, '' ) ).addClass( 'section-' + sItemId );
                    }
                    bChangedLoc = true;
                }
            }else{
    ...
    window.scrollToSection = function( sItem, immediate ){
        var $item = $items.filter( '#story-' + sItem )
            , oData = $item.data()
            ,   top = oData.iTop + ( $items[0] === $item[0] ? 0 : iWindowHeight + 1 );
